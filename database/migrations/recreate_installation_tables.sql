-- Recreate Installation Projects Table with Correct Schema
-- WARNING: This will delete all existing installation project data
-- Only use this if the fix script doesn't work and you're okay with losing data
...
-- Recreate installation_projects table with correct schema
...
-- Recreate installation_work_items table
...
-- Recreate installation_material_usage table
CREATE TABLE public.installation_material_usage (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Recreate installation_work_activities table
CREATE TABLE public.installation_work_activities (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    work_item_id UUID NOT NULL REFERENCES public.installation_work_items(id) ON DELETE CASCADE,
...
-- Create all indexes
CREATE INDEX idx_installation_projects_customer_id ON public.installation_projects(customer_id);
CREATE INDEX idx_installation_projects_status ON public.installation_projects(status);
CREATE INDEX idx_installation_projects_assigned_manager ON public.installation_projects(assigned_manager_id);
CREATE INDEX idx_installation_projects_customer_name ON public.installation_projects(customer_name);
CREATE INDEX idx_installation_projects_location ON public.installation_projects(site_latitude, site_longitude);

CREATE INDEX idx_installation_work_items_project_id ON public.installation_work_items(project_id);
CREATE INDEX idx_installation_work_items_work_type ON public.installation_work_items(work_type);
CREATE INDEX idx_installation_work_items_status ON public.installation_work_items(status);
CREATE INDEX idx_installation_work_items_lead_employee ON public.installation_work_items(lead_employee_id);

CREATE INDEX idx_installation_material_usage_work_item ON public.installation_material_usage(work_item_id);
CREATE INDEX idx_installation_material_usage_material_name ON public.installation_material_usage(material_name);

CREATE INDEX idx_installation_work_activities_work_item ON public.installation_work_activities(work_item_id);
CREATE INDEX idx_installation_work_activities_employee ON public.installation_work_activities(employee_id);
CREATE INDEX idx_installation_work_activities_timestamp ON public.installation_work_activities(timestamp);
CREATE INDEX idx_installation_work_activities_type ON public.installation_work_activities(activity_type);
...
-- Create RLS Policies (same as in main migration)
...
-- Add other RLS policies (abbreviated for space)
...
SELECT 'Installation tables recreated successfully with correct schema' as result;
